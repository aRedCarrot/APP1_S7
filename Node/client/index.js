...
(async function () {
  ...
  console.log(
    `Resolved A record value (Ipv4) "${hostname}" from zone "${zoneName}"`
  );
  ...
  const ShowMainMenu = () => {
    ...
    if (choice === "1") {
      ...
      console.log(
        "Sent your signup information to the server, waiting for reply"
      );
    }
    if (choice === "2") {
      if (SESSION.loggedIn) {
        clientSocket.write(
          `LOGOUT:${SESSION.username}:${SESSION.sessionToken}`
        );
        console.log("Sending logout request ");
      } else {
        const username = prompt(`Enter a username : `);
        const password = prompt.hide(
          `Enter a password (Chars are invisible) : `
        );
        clientSocket.write(`LOGIN:${username}:${password}`);
        console.log(
          "Sent your login information to the server, waiting for reply"
        );
      }
    }
    if (choice === "3") {
      ...
      clientSocket.write(
        `SECRET:${SESSION.username}:${SESSION.sessionToken}:${message}`
      );
